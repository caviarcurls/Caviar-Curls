...
function openProductModal(product) {
  ...
  modalPrice.textContent = product.displayPrice || `Â£${product.price || 0}`;
  ...
  function setupBundleCard(config) {
    ...
    function fillLengthOptions() {
      ...
      Object.keys(prices).forEach((inch) => {
        ...
        option.value = inch;
        option.textContent = `${inch} - Â£${prices[inch]}`;
  ...
  function setupSingleOptionWig(config) {
    ...
    function fillLengths() {
      ...
      Object.keys(config.priceMap).forEach((length) => {
        const option = document.createElement("option");
        option.value = length;
        option.textContent = `${length} - Â£${config.priceMap[length]}`;
        lengthSelect.appendChild(option);
      });
    }
  ...
  function setupVietnameseGalleryWig() {
    ...
    function fillLengths() {
      ...
      Object.keys(prices).forEach((length) => {
        const option = document.createElement("option");
        option.value = length;
        option.textContent = `${length} - Â£${prices[length]}`;
        lengthSelect.appendChild(option);
      });
    }
...
function renderCartPageFromStorage() {
  ...
  if (!Array.isArray(storedCart) || storedCart.length === 0) {
    ...
    subtotalEls.forEach((el) => { el.innerText = 'Â£0.00'; });
    totalEls.forEach((el) => { el.innerText = 'Â£0.00'; });
    ...
    itemsWrap.innerHTML += `
      <div class="drawer-item">
        <div class="drawer-image">
          ...
          <span class="item-confirm">âœ“</span>
        </div>
        ...
        <div class="drawer-info">
          ...
          <p>Type - ${type}</p>
          <p>Length - ${length}"</p>
          <div class="drawer-price">Â£${(price * quantity).toFixed(2)}</div>

          <div class="qty-wrap">
            <button type="button" data-cart-action="decrease" data-index="${index}" aria-label="Decrease quantity">âˆ’</button>
            <span>${quantity}</span>
            <button type="button" data-cart-action="increase" data-index="${index}" aria-label="Increase quantity">+</button>
          </div>

          <button class="remove-item" type="button" data-cart-action="remove" data-index="${index}" aria-label="Remove item">Ã—</button>
        </div>
      </div>
    `;
  });
  ...
  subtotalEls.forEach((el) => { el.innerText = `Â£${subtotal.toFixed(2)}`; });
  totalEls.forEach((el) => { el.innerText = `Â£${subtotal.toFixed(2)}`; });
